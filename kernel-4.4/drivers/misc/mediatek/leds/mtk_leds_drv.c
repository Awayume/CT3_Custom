...
static int mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	LEDS_DRV_DEBUG(" ---- mt65xx_led_set_cust() ---- Here\n");
...
static void mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	LEDS_DRV_DEBUG("mt65xx_led_set: level=%d\n", level);
...
{
	LEDS_DRV_DEBUG(" ---- mt65xx_blink_set()\n");
...
{
	struct cust_mt65xx_led *cust_led_list = mt_get_cust_led_list();
	/*LEDS_DRV_DEBUG("---- backlight_brightness_set() ----\n");*/
	...
	if (cust_led_list[MT65XX_LED_TYPE_LCD].mode == MT65XX_LED_MODE_CUST_BLS_PWM) {
#ifdef CONTROL_BL_TEMPERATURE
		mutex_lock(&bl_level_limit_mutex);
		...
		if (limit_flag == 0) {
			...
			if (limit < current_level) {
				...
				level =
				    (limit << (MT_LED_INTERNAL_LEVEL_BIT_CNT - 8)) | (limit >>
										      (16 -
		...
		if (bl_setting->mode == MT65XX_LED_MODE_PMIC) {
			...
			if (level == 0) {
				...
				mt_backlight_set_pwm_duty(bl_setting->data,
							  level, bl_div, &bl_setting->config_data);
			}
		}
...
static void mt65xx_leds_shutdown(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		case MT65XX_LED_MODE_CUST_EXT:
			LEDS_DRV_DEBUG("LED control through EXT!!1\n");
			mt_cust_set_led_brightness(g_leds_data[i]->cust.data, 0);
			break;
